...
export default function ContentsPage(){
  ...
  const visible=useMemo(()=>data?.filter(c=>(filter==="all"||c.status===filter)&&(`${c.title} ${c.body}`.toLowerCase().includes(query.toLowerCase()))).sort((a,b)=>(sortOrder==="newest"?-1:1)*(+new Date(a.createdAt)-+new Date(b.createdAt)))||[],[data,filter,query,sortOrder]);
  return <><PageHeader title="콘텐츠" action={<Link href="/contents/new" className="focus-ring hidden min-h-11 items-center gap-2 rounded-xl bg-[#ef6b32] px-4 text-sm font-black text-white sm:flex"><Plus className="size-4"/>새로 만들기<span className="rounded-md bg-white/20 px-1.5 py-0.5 text-[10px] tracking-wide">AI</span></Link>}/><div className="mx-auto max-w-4xl">
    <div className="mb-5 flex flex-col gap-3"><div className="flex gap-2"><label className="relative flex-1"><Search className="absolute left-4 top-3.5 size-4 text-stone-400"/><input value={query} onChange={e=>setQuery(e.target.value)} className="field pl-11" placeholder="빵 이름이나 글 내용 검색"/></label><label className="relative w-32 shrink-0"><span className="sr-only">정렬 방식</span><select value={sortOrder} onChange={e=>setSortOrder(e.target.value as "newest"|"oldest")} className="field appearance-none pr-9 text-sm font-semibold"><option value="newest">최신순</option><option value="oldest">오래된순</option></select><ChevronDown className="pointer-events-none absolute right-3 top-4 size-4 text-stone-400"/></label></div><div className="hide-scrollbar -mx-4 flex gap-2 overflow-x-auto px-4 pb-1 sm:mx-0 sm:px-0">{filters.map(item=><button key={item.value} onClick={()=>setFilter(item.value)} className={cn("focus-ring min-h-10 shrink-0 rounded-full px-4 text-sm font-bold transition",filter===item.value?"bg-stone-900 text-white":"border border-stone-200 bg-white text-stone-500 hover:border-stone-300")}>{item.label}{data&&<span className="ml-1 opacity-60">{item.value==="all"?data.length:data.filter(c=>c.status===item.value).length}</span>}</button>)}</div></div>
