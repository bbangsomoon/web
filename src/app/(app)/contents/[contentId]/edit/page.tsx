...
import { CalendarClock, Plus, X, Zap } from "lucide-react";
...
import { getThumbnailText, isThumbnailRequested, setThumbnailText as persistThumbnailText } from "@/lib/content-thumbnail";
...
function ContentEditor({ data, storeId }: { data: Content; storeId: string }) {
  ...
  const requestedInGeneration = isGenerationReview && searchParams.get("thumbnail") === "1";
  const [storedThumbnailRequested, setStoredThumbnailRequested] = useState(false);
  const [thumbnailText, setThumbnailText] = useState("");
  const thumbnailRequested = requestedInGeneration || storedThumbnailRequested;
  useEffect(() => {
    setStoredThumbnailRequested(isThumbnailRequested(id));
    setThumbnailText(getThumbnailText(id));
  }, [id]);
  ...
  return <div className="mx-auto max-w-4xl">
    ...
    <div className="grid items-start gap-5 lg:grid-cols-[minmax(280px,.8fr)_1.2fr]">
      ...
      <section className="surface rounded-[28px] p-5 sm:p-7">
        {thumbnailRequested && <label className="mb-7 block">
          <span className="mb-2 block text-sm font-bold">AI 썸네일 문구</span>
          <input value={thumbnailText} onChange={(event) => { setThumbnailText(event.target.value); persistThumbnailText(id, event.target.value); }} className="field" placeholder="예: 오늘만 20% 할인" />
        </label>}
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-bold">피드 내용</h2>
        </div>
        <textarea value={body} onChange={(event) => setBody(event.target.value)} rows={12} className="field resize-none leading-7" aria-label="피드 내용" />
        ...
        <div className="mt-7">
          <h3 className="text-sm font-black">해시태그</h3>
        ...
        <div className="mt-8 border-t border-stone-200 pt-7">
          <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400 text-white"><InstagramIcon className="size-5" /></span><div><h3 className="text-sm font-black">인스타그램 피드 게시 방식</h3><p className="mt-1 text-xs text-stone-500">{social.data?.username ? `@${social.data.username}` : "연결된 계정 없음"}</p></div></div>
        ...
        <div ref={publishActionsRef} className="mt-8 grid gap-2 sm:grid-cols-2">
