...
export default function PublishPage() {
  ...
  const publish=useMutation({mutationFn:()=>mockApi.publishContent(id,mode,mode==="scheduled"?new Date(`${date}T${time}`).toISOString():undefined),onSuccess:(next)=>{client.setQueryData(["content",id],next);client.invalidateQueries({queryKey:["contents"]});toast(mode==="now"?"Instagram 피드에 게시했어요!":"게시 시간을 예약했어요!");router.push(`/contents/${id}`)}});
  ...
  return <div className="mx-auto max-w-3xl"><PageHeader title="게시 설정" description="언제 인스타그램에 소문낼지 선택해 주세요." backHref={`/contents/${id}/edit`}/>
    <div className="surface overflow-hidden rounded-[28px]"><div className="grid sm:grid-cols-[150px_1fr]"><div className="relative aspect-video sm:aspect-square"><MediaPreview asset={content.data.assets[0]} sizes="300px" controls={content.data.assets[0]?.type==="video"}/></div><div className="p-5"><span className="text-xs font-black text-stone-400">게시할 인스타그램 피드</span><p className="mt-2 line-clamp-3 text-sm font-semibold leading-6 text-stone-600">{content.data.body}</p></div></div></div>
    ...
    </section><Button className="mt-5 w-full min-h-14 text-base" onClick={()=>publish.mutate()} disabled={publish.isPending}>{publish.isPending?"게시를 준비하고 있어요":mode==="now"?"인스타그램 피드에 게시하기":`${date} ${time}에 예약하기`}</Button><p className="mt-3 text-center text-xs leading-5 text-stone-400">MVP에서는 실제 인스타그램에 게시되지 않고 상태만 변경됩니다.</p>
  </div>;
}
