  ...
  return <>
    ...
    <section className="surface overflow-hidden rounded-[28px]">
      ...
      <div className="p-5 sm:p-7">
        ...
        <Button onClick={() => toggle.mutate()} disabled={toggle.isPending} variant={data.connected ? "secondary" : "primary"} className="mt-5 w-full">{toggle.isPending ? <LoaderCircle className="size-4 animate-spin" /> : data.connected ? <Unlink className="size-4" /> : <InstagramIcon className="size-4" />}{data.connected ? "연결 해제" : "인스타그램 연결"}</Button>
      </div>
    </section>
