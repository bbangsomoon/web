...
export default function DashboardPage() {
  ...
  const { data, isLoading, isError } = useQuery({ queryKey: ["contents", storeId, "dashboard"], queryFn: () => contentApi.getContents(storeId, { size: 100 }), enabled: Boolean(storeId) });
  if (stores.isLoading || isLoading) return <LoadingState/>; if (stores.isError || isError || !data || !storeId) return <ErrorState/>;
