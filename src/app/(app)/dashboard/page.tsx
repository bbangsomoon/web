...
import { ErrorState, LoadingState, ServiceNotice } from "@/components/common/ui";
...
export default function DashboardPage() {
  ...
  const contentsQuery = useQuery({ queryKey: ["contents", storeId, "dashboard"], queryFn: () => contentApi.getContents(storeId, { size: 100 }), enabled: Boolean(storeId), retry: 2, refetchInterval: (query) => query.state.status === "error" ? 10_000 : false });
  if (stores.isLoading || !storeId) return <LoadingState/>; if (stores.isError) return <ErrorState/>;
