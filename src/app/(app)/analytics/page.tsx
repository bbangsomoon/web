...
export default function AnalyticsPage() {
  ...
  if (isLoading) return <div className="mx-auto max-w-5xl"><PageHeader title="성과" /><LoadingState /></div>;
  if (isError || !data) return <div className="mx-auto max-w-5xl"><PageHeader title="성과" /><ErrorState /></div>;
  ...
  return <div className="mx-auto max-w-5xl">
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-[1.5fr_1fr]">
      <section className="surface rounded-[28px] p-5 sm:p-7"><div className="flex items-center justify-between"><div><span className="text-xs font-black text-stone-400">게시물별 조회</span><h2 className="mt-1 text-xl font-black">잘 퍼진 소문</h2></div><TrendingUp className="size-6 text-emerald-600" /></div><div className="mt-7 space-y-5">{published.map((content) => <div key={content.id}><div className="mb-2 flex justify-between gap-4 text-sm"><b className="line-clamp-1">{content.body}</b><span className="shrink-0 font-black">{content.insight?.views.toLocaleString()}회</span></div><div className="h-2 overflow-hidden rounded-full bg-stone-100"><div className="h-full rounded-full bg-gradient-to-r from-[#ef6b32] to-[#f4aa65]" style={{ width: `${((content.insight?.views || 0) / max) * 100}%` }} /></div></div>)}</div></section>
